...
//extended: 중첩된 객체 표현을 허용할지 말지를 정함
...
app.get('/getForm', (req, res) => {
  ...
  res.render('result', {
    title: 'GET요청 결과 확인',
    userInfo: { id: id, pw: pw },
  });
  //key-value형태에서 key값과 value의 변수가 동일할 때는 value를 생략 가능
});

app.post('/postForm', (req, res) => {
  //post 방식은 request(req)의 bodyy 형태로 접근
...
//예를 들어 get 방식의 '/login'과 post방식의 '/login'은 다른 페이지지만
//use는 동일한 페이지로 인식
//use는 404 페이지일 때 사용
app.use('*', (req, res) => {
  res.render('404');
